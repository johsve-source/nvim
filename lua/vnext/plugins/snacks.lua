    ...
    vim.api.nvim_create_autocmd("User", {
      ...
      callback = function()
        ...
        Snacks.toggle.option("autoformat", {
          ...
          set = function(val) vim.g.autoformat = val end,
          notify = function(val)
            local red_autoformat = "\27[31mAutoformat\27[0m"
          vim.notify(
            (val and "Enabled " or "Disabled ") .. red_autoformat,
            vim.log.levels.WARN,
            { title = "Snacks", timeout = 2000 }
          )
          end,
        }):map("<leader>tF")
